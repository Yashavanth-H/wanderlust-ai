...
export default function Preferences() {
      ...
      if (data.status === "success" && Array.isArray(data.recommendations)) {
        ...
        const formattedDestinations = data.recommendations.map(
          (dest: {
            ...
            id: dest.id ? dest.id : String(Math.floor(Math.random() * 1000000)),
            name: dest.name,
            ...
            facts: [
              ...
              `Best visited during ${Array.isArray(dest.best_season)
                ? dest.best_season.join(", ")
                : "any season"
              }`,
              `Popular for ${Array.isArray(dest.activities)
  ...
  const SelectionCard = ({
    ...
    return (
      <div
        className={`bg-background rounded-xl p-6 cursor-pointer shadow-md relative overflow-hidden ${isSelected ? "ring-4 ring-black-400" : ""
          }`}
        ...
        return (
          ...
          <div>
            <div className="flex justify-between items-center mb-8">
              ...
              <button
                className={`text-sm px-4 py-2 rounded-lg transition-all ${comparison
                ...
                <TabsContent value="all" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {recommendedDestinations.map((destination, index) => (
                    ...
                    <DestinationCard
                      ...
                      image={destination.image} // Real image URL from API

                      rating={destination.rating} // Rating (e.g., 4.5)

                      matchPercentage={99} // Static match percentage for now
                      index={index} country={destination.country} tags={destination.tags} />
                  ))}
                </TabsContent>
              </Tabs>
            )}
        ...
        <div className="min-h-[400px] flex flex-col justify-between">
          ...
          {step < 5 && (
            <div className="mt-8 flex justify-end">
              ...
              <button
                className={`px-6 py-2 rounded-lg flex items-center hover:scale-105 active:scale-95 transition-transform ${isStepComplete()
